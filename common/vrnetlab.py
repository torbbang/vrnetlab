...
class VM:
    ...
    def __init__(
        ...
        # Use "defer" so we can configure multifd/mapped-ram on the monitor before
        # pulling the saved state in via migrate_incoming (see start()).
        if restoring_snapshot:
    ...
    def start(self):
        ...
        # qemu was launched with "-incoming defer", so configure the same multifd /
        # mapped-ram parameters used when saving before kicking off migrate_incoming.
        snapshot_state = f"/snapshot-data/vm{self.num}/state.img"
    ...
    def snapshot_save_to_dir(self, vm_dir):
        ...
        try:
            ...
            # Start migration. multifd + mapped-ram parallelise the state dump across
            # CPU cores. The same parameters are reapplied on the restore side in
            # start(). Note: mapped-ram is incompatible with multifd compression/TLS,
            # so compression is intentionally not used here.
            ...
            # Start copying disks in parallel with the state migration.
            # Disks are safe to copy because the VM is paused.
...
class VR:
    ...
    def snapshot_save(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Create tar. "-S" preserves sparseness so the (mostly-zero) state and
            # qcow2 files don't balloon on disk.
